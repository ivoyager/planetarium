...
func _ready():
	...
	connect("item_rect_changed", Callable(self, "_on_self_item_rect_changed"))
	for child in get_parent().get_children():
		var control := child as Control
		if !control or control == self:
			continue
		_other_panels.append(control)
		control.connect("item_rect_changed", Callable(self, "_resize_vertical"))
	_selection_data.connect("resized", Callable(self, "_resize_vertical"))
	get_viewport().connect("size_changed", Callable(self, "_resize_vertical"))
